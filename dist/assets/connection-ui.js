...
export function panelToolsStatus(cloud,runtime={}) {
  return `<section class="panel autonomy-summary"><h2>Telas e ferramentas dentro do painel</h2><p>${runtime.available?'O serviço online está acessível. Abra a tela da conta, entre e confirme qual perfil o Astra pode operar.':'O acesso às telas e ao editor está preparado. Falta ativar o serviço online na hospedagem para usá-los com seu computador desligado.'}</p><div class="company-links"><a class="p-button secondary" href="#/browser">Instagram, WhatsApp, Facebook e Google ↗</a><a class="p-button secondary" href="#/video">Astra Vídeo ↗</a></div><p>As telas das contas e as conexões por API são acessos separados. O login e as verificações de segurança das contas são feitos por você.</p></section>`;
}
