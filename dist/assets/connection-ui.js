...
export function autonomySummary(state,esc) {
  ...
  return `<section class="panel autonomy-summary"><h2>O que o Astra pode fazer sozinho</h2><dl><div><dt>Criar pelo chat</dt><dd>Em Conversar e executar, seu pedido autoriza criar textos e imagens. Você não precisa aprovar a geração novamente. Limites, materiais exigidos e proibições continuam valendo.</dd></div><div><dt>Criar sem abrir uma conversa</dt><dd>${esc(routine)}</dd></div><div><dt>Publicar</dt><dd>${p.allowPublishing?'Envio de conteúdos aprovados habilitado.':'Envio de conteúdos aprovados desativado.'} A versão atual exige revisar e aprovar a peça e autorizar a publicação. Conectar a conta não aprova conteúdos.</dd></div><div><dt>Responder mensagens</dt><dd>${p.autoReply?'Respostas automáticas habilitadas; dependem do canal, das regras e de informações suficientes.':'Respostas automáticas desativadas.'}</dd></div><div><dt>Editar vídeos</dt><dd>O Astra Vídeo ainda precisa ser integrado. Por enquanto, a Helpu prepara o roteiro e o plano de edição.</dd></div></dl></section>`;
}

export function panelToolsStatus(cloud) {
  return `<section class="panel autonomy-summary"><h2>Telas e ferramentas dentro do painel</h2><p>${cloud?'As telas do Instagram, WhatsApp, Facebook e Google ainda não estão disponíveis nesta versão online. Elas precisam de um navegador persistente conectado à Helpu.':'As sessões locais podem ser abertas neste computador. A prévia no painel ainda não oferece controle de mouse e teclado.'}</p><p>O editor Astra Vídeo também ainda não está integrado. Conectar uma API permite usar suas funções autorizadas, mas não faz a tela do serviço aparecer aqui.</p></section>`;
}
