export function createBillingUI({esc,getState}){
 ...
 function launchOffer(b){const p=b.launchOffer;if(!p||b.subscription)return '';return '<p class="account-test"><strong>Primeiro mês: '+esc(money(p.firstAmount,p.currency))+'</strong><br>Depois, '+esc(money(p.regularAmount,p.currency))+' por mês. Desconto aplicado somente à primeira cobrança; confira o total no Stripe.</p>';}
 function services(b){if(!b.services?.length)return '';return '<h3>Serviços avulsos</h3>'+b.services.map(o=>'<div class="account-empty"><strong>'+esc(o.price.name)+'</strong><p class="account-price">'+priceText(o.price)+'</p><p>'+ (o.paymentStatus==='paid'?'Pagamento confirmado. A equipe Helpu acompanha a entrega.':o.paymentStatus==='pending'?'Pagamento aguardando confirmação. Não é necessário pagar novamente.':'Contratação única. O escopo e a entrega são acompanhados pela equipe Helpu.')+'</p>'+(o.canCheckout?'<button class="p-button secondary" data-account-action="checkout" data-offer-id="'+esc(o.id)+'">Contratar pelo Stripe ↗</button>':'')+'</div>').join('');}
