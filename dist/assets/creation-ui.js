...
export function createCreationUI({api, endpoint, getState, esc, toast, uploadFile, refresh}) {
 ...
 let styles=[],presets=[],techniques=[],styleId='',videoOptions={aspectRatio:'9:16',quality:'high',preset:'clean'},referenceOnlyIds=[],reviewRequests=new Map();
 ...
 function ensureCompany() {
  ...
  styles=[];presets=[];techniques=[];styleId='';videoOptions={aspectRatio:'9:16',quality:'high',preset:'clean'};referenceOnlyIds=[];reviewRequests.clear();
 ...
 function reviewMarkup(item){return `<section class="creation-review"><p><strong>${item.review==='approved'?'Aprovado':item.review==='changes_requested'?'Ajustes solicitados':'Prévia para aprovação'}</strong>${item.revisionOf?' · Nova versão':''}</p><div class="creation-attachment-actions"><button class="p-button primary" data-creation-action="approve" data-id="${esc(item.id)}" ${locked()||item.review==='approved'?'disabled':''}>Aprovar prévia</button>${item.format==='reels'?`<button class="p-button secondary" data-creation-action="reuse" data-id="${esc(item.id)}">Criar no mesmo estilo</button>`:''}</div><label class="p-field"><span>O que deseja ajustar?</span><textarea data-review-text="${esc(item.id)}" maxlength="4000" placeholder="Ex.: reduza o texto e deixe a primeira cena mais lenta."></textarea></label><button class="p-button secondary" data-creation-action="revise" data-id="${esc(item.id)}" ${locked()?'disabled':''}>Gerar versão ajustada</button><p class="micro-copy">A versão anterior permanece disponível. O ajuste usa uma nova geração.</p>${item.format==='reels'&&item.review==='approved'?`<label class="p-field"><span>Nome para guardar este estilo</span><input data-style-name="${esc(item.id)}" maxlength="80" placeholder="Ex.: Tour elegante da minha empresa"></label><button class="p-button secondary" data-creation-action="save-style" data-id="${esc(item.id)}">Salvar meu estilo</button>`:''}</section>`;}
 ...
 async function load() {
  ...
  try {
   ...
   const changed=JSON.stringify(styles)!==JSON.stringify(result.styles||[])||!presets.length;styles=result.styles||[];presets=result.capabilities?.videoPresets||[];techniques=result.capabilities?.techniques||[];if(changed&&!locked())paintForm();
 ...
 async function onSubmit(event) {
  ...
  const body = {prompt, format, attachments: attachments.map(a => a.id), ...(format === 'carousel' ? {slideCount} : {}), ...(format === 'reels' ? {duration,videoOptions,styleId:styleId||undefined,referenceOnlyIds:referenceOnlyIds.filter(id=>attachments.some(a=>a.id===id))} : {})};
 ...
 async function onClick(event) {
  ...
  if(action==='reuse'){
    ...
    format='reels';styleId='';videoOptions={...item.videoOptions,musicAssetId:null};duration=item.duration||15;attachments=[];referenceOnlyIds=[];pending=null;paintForm();$('#creation-prompt')?.focus();toast('Estilo aplicado. Envie os novos materiais e descreva o pedido.');return;
  }
