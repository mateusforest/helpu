...
export function openBrowser(url) {
  const child = process.platform === 'win32'
    ? spawn('powershell.exe', ['-NoProfile', '-NonInteractive', '-WindowStyle', 'Hidden', '-Command', 'Start-Process -FilePath $env:HELPU_OPEN_URL'],
      {env: {...process.env, HELPU_OPEN_URL: url}, windowsHide: true, stdio: 'ignore'})
...
export async function startHelpu({open = false, openUrl = openBrowser} = {}) {
  ...
  // Reserve the port before opening SQLite: a second launch must not recover live jobs.
  ...
  try {
    ...
    app = createHelpuServer({dataDir: config.dataDir});
  } finally {
  ...
  if (open) {
    ...
    catch { console.warn(`Não foi possível abrir o navegador automaticamente. Acesse ${url}`); }
  }
  return {app, shutdown, config};
}
...
export async function run() {
  ...
  process.on('message', message => { if (message?.type === 'helpu:shutdown') stop(); });
  process.on('disconnect', stop);
  try {
    runtime = await startHelpu({open: process.argv.includes('--open')});
    if (stopRequested) stop();
    else if (process.connected) process.send({type: 'helpu:ready', origin: runtime.config.origin});
    return runtime;
  } catch (error) {
