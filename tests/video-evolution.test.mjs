...
test('real landscape export preserves length through transitions, mixes music and samples references', {skip:!process.env.HELPU_TEST_FFMPEG||!process.env.HELPU_TEST_FFPROBE,timeout:160000},async t=>{
 ...
 await command(ffmpeg,['-v','error','-y','-f','lavfi','-i','testsrc2=size=480x270:rate=30:duration=3','-c:v','libx264','-pix_fmt','yuv420p',source],{windowsHide:true});
