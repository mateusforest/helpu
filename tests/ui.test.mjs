import {createDisclosureState} from '../dist/assets/disclosure-state.js';
...
test('todas as áreas e formulários produzem HTML sem erros de execução',()=>{
 ...
 const routes=vm.runInContext('routes.map(x=>x[0])',ctx);assert.equal(routes.length,30);assert.ok(routes.includes('websites'));
 ...
 for(const route of ['company','brand','settings','integrations','overview','agents','leads','pages']){vm.runInContext('view='+JSON.stringify(route)+';render();',ctx);assert.doesNotMatch(nodes.get('#workspace').innerHTML,/name="dailyRuns"|name="dailyMedia"|name="publicBaseUrl"|Uso interno|Configurar Astra|data-action="company-diagnosis"|Outros recursos da empresa|Sua equipe de agentes/);}
 vm.runInContext("view='settings';render();",ctx);assert.match(nodes.get('#workspace').innerHTML,/name="schedulesEnabled"/);assert.doesNotMatch(nodes.get('#workspace').innerHTML,/name="enabled"|name="autoMedia"/);
 vm.runInContext("view='agents';render();",ctx);assert.equal(vm.runInContext('view',ctx),'consultations');
