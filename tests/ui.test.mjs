...
test('todas as áreas e formulários produzem HTML sem erros de execução',()=>{
 ...
 const ctx=vm.createContext({document:doc,window:{addEventListener(){}},location:{hash:'',origin:'http://localhost',assign(){}},localStorage:{getItem(){return null;},setItem(){}},setInterval(){},setTimeout(){},clearTimeout(){},__fixture:fixture,Date,Intl,URL,console,crypto:{randomUUID(){return 'test';}},createConversationUI:()=>({chat:()=>'<h1>Conversa</h1>',accounts:()=>'<h1>Contas</h1>',mount(){}})});
 const code=fs.readFileSync(new URL('../dist/assets/portal.js',import.meta.url),'utf8').replace(/^import[^\n]*\n/,'').replace(/init\(\);\s*$/,'');vm.runInContext(code,ctx);
