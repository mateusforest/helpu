...
test('agendamento respeita data, recorrência, pausa, cancelamento, isolamento e remoção de acesso',async()=>{
 ...
 const db={prepare:s=>raw.prepare(s)};let clock=Date.parse('2026-09-18T10:00:00Z');const calls=[];const policy={enabled:false,schedulesEnabled:true,timeZone:'America/Sao_Paulo'};
 const service=createCreationSchedules({db,now:()=>clock,company:async()=>({policy}),creations:{submit:async(...args)=>{calls.push(args);return {id:'creation-'+calls.length};}}});
 ...
 clock=Date.parse(input.scheduledAt);policy.schedulesEnabled=false;policy.enabled=true;await service.tick();assert.equal(calls.length,0);await assert.rejects(service.create('org',1,input,'chat'),/Preferências/);policy.schedulesEnabled=true;policy.enabled=false;await service.tick();await service.tick();assert.equal(calls.length,1);assert.equal((await service.list('org',1))[0].enabled,false);
...
test('agendamentos de Reels preservam oito referências e recusam excesso ou duplicidade antes de salvar',async t=>{
 ...
 const db={prepare:s=>raw.prepare(s)};let clock=Date.parse('2026-09-18T10:00:00Z');const calls=[];const policy={enabled:false,schedulesEnabled:true,timeZone:'America/Sao_Paulo'};
 const service=createCreationSchedules({db,now:()=>clock,company:async()=>({policy}),creations:{submit:async(...args)=>{calls.push(args);return {id:'scheduled-reels'};}}});
