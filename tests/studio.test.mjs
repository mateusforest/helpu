...
test('rotina prioriza entregas, bloqueios, execuções e resultados antes de novas ideias', () => {
  for (const status of ['draft', 'review', 'approved', 'scheduled']) assert.equal(dailyPriority({
    contents: [{
      id: 'existing',
      status
    }]
  }).intelligenceRequired, false);
  for (const state of ['producing', 'awaiting_approval', 'blocked', 'measuring']) assert.equal(dailyPriority({
    operations: [{
      id: 'op',
      state
    }]
  }).intelligenceRequired, false);
  assert.equal(dailyPriority({
    operations: [{
      id: 'op',
      state: 'completed',
      result: {}
  ...
  assert.equal(dailyPriority({
    ...
    jobs: [{
      state: 'blocked',
      idempotency_key: 'daily-director:old'
    }]
...
test('produção existente: origem, dependências e evidência sem geração fictícia', async t => {
  ...
  const pieces = ['carousel', 'video', 'image'].map(format => ({
    title: 'TEST ONLY ' + format,
    caption: 'Texto original confirmado no fixture.',
    visualPrompt: 'Briefing original do fixture.',
    format,
    channel: 'instagram'
  }));
  const providers = {
    ...
    media: async () => {
      mediaCalls++;
      throw new Error('External generation forbidden in tests');
    }
  };
  const open = async () => {
  ...
  try {
    await open();
    db = new DatabaseSync(path.join(directory, 'helpu.sqlite'));
    ...
    await api('company', 'PATCH', {
      profile: {
        ...
        audience: 'Teste.',
        visualIdentity: 'Geist, verde e branco.',
        tone: 'Objetivo.'
      }
    });
    await api('jobs', 'POST', {
      ...
      payload: {
        agent: 'creative',
        brief: 'Fixture: três rascunhos.'
      },
      idempotencyKey: 'fixture-source'
    });
    ...
    await t.test('verificação escolhe a entrega existente e comprova materiais e executor ausentes', async () => {
      const result = await api('studio/' + content.id + '/preflight', 'POST', {});
      assert.equal(result.status, 200);
      first = result.body;
      operationId = first.operationId;
      assert.equal(first.contentId, content.id);
      ...
      const s = await state();
      assert.equal(s.records.content.length, 3);
      assert.equal(s.operations.length, 1);
      assert.equal(s.records.tasks.length, 1);
      assert.ok(s.records.content.every(c => c.operationId === operationId));
      assert.ok(s.records.content.every(c => c.status === 'review' && c.caption === content.caption));
      assert.equal(s.operations[0].state, 'blocked');
      assert.equal(s.records.tasks[0].status, 'todo');
      assert.equal(s.records.campaigns.length, 0);
      assert.equal(s.operations[0].evidence[0].status, 'recorded');
      assert.equal(s.operations[0].evidence[0].executor, 'local');
    });
    await t.test('repetição preserva os mesmos registros, versão, tarefa e histórico', async () => {
      ...
      assert.equal(again.storedVersion, 1);
      assert.deepEqual([count('records'), count('conversation_messages'), count('audit')], before);
    });
    await t.test('aprovação, geração e publicação não atravessam as dependências', async () => {
      for (const action of ['approve', 'execute', 'schedule']) assert.equal((await api('operations/' + operationId + '/action', 'POST', {
        action
      })).status, 409);
      for (const kind of ['image', 'video', 'publish']) assert.equal((await api('jobs', 'POST', {
        kind,
        payload: {
          contentId: content.id
        }
      })).status, 409);
      await assert.rejects(async () => await server.portal.kernel.action(org, operationId, 1, {
        action: 'approve'
      }));
    ...
    await t.test('retomada usa a verificação existente sem chamar o diretor', async () => {
      assert.equal((await api('operations/' + operationId + '/action', 'POST', {
        action: 'pause'
      })).status, 200);
      assert.equal((await api('operations/' + operationId + '/action', 'POST', {
        action: 'resume'
      })).status, 200);
      assert.equal((await state()).operations[0].state, 'blocked');
      assert.equal(aiCalls, 1);
      assert.equal((await state()).records.content.length, 3);
    ...
    await t.test('mudança de briefing cria nova verificação e preserva a anterior', async () => {
      ...
      await api('records/content/' + content.id, 'PATCH', {
        version: current.version,
        visualPrompt: 'Nova direção fornecida no fixture.'
      });
      assert.equal((await api('studio/' + content.id)).body.stale, true);
      const changed = (await api('studio/' + content.id + '/preflight', 'POST', {})).body;
      assert.equal(changed.storedVersion, 2);
      ...
      assert.equal(previous.briefing.visualPrompt, content.visualPrompt);
    ...
    await t.test('rotina não gera mais três rascunhos e não reserva chamada de IA', async () => {
      ...
      await server.portal.tick();
      const s = await state();
      assert.equal(s.routine.status, 'pending_work');
      assert.equal(s.routine.contentIds.length, 3);
      assert.equal(s.company.policy.dailyRuns, 8);
      assert.equal(aiCalls, calls);
      assert.equal(count('usage_reservations'), usage);
      assert.equal(s.records.content.length, 3);
      assert.equal(s.jobs.filter(j => j.payload.agent === 'director').length, 0);
    });
    await t.test('job diário antigo enfileirado é barrado antes da inteligência', async () => {
      await api('jobs', 'POST', {
        ...
        idempotencyKey: 'daily-director:fixture-queued'
      });
      await server.portal.tick();
      const job = (await state()).jobs.find(j => j.payload.agent === 'director');
    ...
    await t.test('outra empresa não consulta nem reutiliza a entrega', async () => {
      const second = (await request('/api/portal/companies', 'POST', {
        name: 'Outra fixture'
      })).body;
    ...
    await t.test('falha ao localizar a origem não deixa ordem órfã', async () => {
      const c = (await api('records/content', 'POST', {
        title: 'Sem origem de agente',
        format: 'image',
        channel: 'instagram'
      })).body;
      const before = (await state()).operations.length;
      assert.equal((await api('studio/' + c.id + '/preflight', 'POST', {})).status, 409);
    ...
    await t.test('reinício preserva origem, versões, evidências e bloqueios sem gerar', async () => {
      const before = (await api('studio/' + content.id)).body;
      await close();
      await open();
      const after = (await api('studio/' + content.id)).body;
      assert.equal(after.sourceHash, before.sourceHash);
      assert.equal(after.storedVersion, before.storedVersion);
      assert.equal(after.operationId, operationId);
      assert.equal(mediaCalls, 0);
      assert.equal((await state()).records.metrics.length, 0);
      assert.equal((await state()).assets.length, 0);
    });
  } finally {
    db?.close();
    await close();
