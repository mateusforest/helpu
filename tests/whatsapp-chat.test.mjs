...
for(const postgres of [false,true])test('WhatsApp oficial: telefone, conversa e isolamento em '+(postgres?'PostgreSQL':'SQLite'),async t=>{
 ...
 const server=await createHelpuServer({dataDir,database,portalOptions:{startScheduler:false,whatsappChatEnv:env,openaiEnv:{OPENAI_API_KEY:'fake-openai'},whatsappChatFetch:async(url,request)=>{if(url.endsWith('/media'))return {ok:true,json:async()=>({id:'uploaded-media'})};sent.push({url,body:JSON.parse(request.body)});if(failSend)throw new Error('Timeout');return {ok:true,json:async()=>({messages:[{id:'sent-'+sent.length}]})};},conversationRespond:async()=>({status:'completed',output:[{type:'message',content:[{type:'output_text',text:'Seu plano foi preparado.'}]}]})}});
 ...
 let count=0;
 const webhook=async(text,from='5511988887777',{valid=true,phoneId='official-phone',id, timestamp=String(Math.floor(Date.now()/1000))}={})=>{
  const value={metadata:{phone_number_id:phoneId,display_phone_number:env.HELPU_WHATSAPP_NUMBER},messages:[{id:id||'wamid-'+(++count),from,type:'text',text:{body:text},timestamp}]},body=JSON.stringify({entry:[{changes:[{value}]}]});
