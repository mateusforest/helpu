...
const server=await createHelpuServer({dataDir,portalOptions:{startScheduler:false,openaiEnv:{OPENAI_API_KEY:'local-test-key'},creationRespond:async()=>({status:'completed',output:[{content:[{type:'output_text',text:JSON.stringify({caption:'Prévia de teste local.',slides:Array.from({length:3},(_,i)=>({title:'Cena '+(i+1),text:'Mensagem de teste '+(i+1),visualPrompt:'Teste sintético',background:'#ffffff',textColor:'#002200'}))})}]}]}),reelsRenderer:{configured:()=>true,render:async()=>({bytes:fixture,width:1920,height:1080,duration:15,sha256:'fixture'})}}});
...
try{
 ...
 await page.locator('summary').filter({hasText:'Biblioteca de modelos de vídeo'}).click();
 assert.equal(await page.locator('.video-recipe-card').count(),7);
 await page.locator('[data-creation-action=recipe][data-id=teaser]').click();
 assert.equal(await page.locator('#creation-style').inputValue(),'preset:teaser');
 ...
 await page.locator('#creation-subtitles').fill('1\n00:00:00,000 --> 00:00:02,000\nLegenda de teste');
 await page.locator('#creation-font').selectOption('serif-italic');
 await page.locator('#creation-fit').selectOption('blur');
 await page.locator('#creation-transition').selectOption('circleopen');
