...
test('autonomia distingue autorização da criação, rotina, publicação e ferramentas pendentes',()=>{
 ...
 let html=autonomySummary(state,esc);assert.match(html,/não há execução recente confirmada/);assert.match(html,/revisar e aprovar/);assert.match(html,/não precisa aprovar a geração novamente/);assert.match(html,/Vídeo ainda precisa ser integrado/);
 state.worker.running=true;html=autonomySummary(state,esc);assert.match(html,/preparar conteúdos e gerar imagens/);
 assert.match(panelToolsStatus(true),/ainda não estão disponíveis nesta versão online/);
});
