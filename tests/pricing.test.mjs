...
for(const postgres of [false,true])test('F10 fluxo administrativo em '+(postgres?'PostgreSQL':'SQLite'),async t=>{
 ...
 try{
  ...
  await t.test('acesso da equipe, empresa de origem, CSRF e privacidade',async()=>{
   assert.equal((await request('/api/portal/pricing')).status,401);assert.equal((await api(undefined,client)).status,403);assert.equal((await request('/api/portal/pricing',operator,{action:'save'},true)).status,403);
  ...
  await t.test('pré-pagos aguardam saldo, arquivar revoga base e nada altera cobrança ou quotas',async()=>{
   ...
   assert.equal((await ok(request('/api/portal/commercial-admin',operator))).pricingSources.length,0);env.HELPU_OPERATOR_USER_IDS='';assert.equal((await api()).status,403);
  });
