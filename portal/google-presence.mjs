...
export function googlePresence(request,tokenFor,bearer){
 ...
 return async function(config,action,input={},requestId){
  ...
  if(action==='update'){const body={profile:{description:String(input.description||'').slice(0,750)}};let updateMask='profile';if(input.website){if(!publicUrl(input.website))throw new ProviderError('Informe um site HTTPS válido.');body.websiteUri=input.website;updateMask+=',websiteUri';}return request(base+location(config)+'?updateMask='+updateMask,{method:'PATCH',headers,body,uncertain:true});}
  if(action==='create'){
