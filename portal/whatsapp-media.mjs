...
const formats={'audio/mpeg':['.mp3',16],'audio/wav':['.wav',16],'audio/ogg':['.ogg',16],'image/png':['.png',20],'image/jpeg':['.jpg',20],'image/webp':['.webp',20],'video/mp4':['.mp4',25],'application/pdf':['.pdf',20]};
export async function receiveWhatsAppMedia({media,token,phoneId,version='v24.0',fetcher=fetch}){
  ...
  const info=await meta.json(),mime=String(info.mime_type||'').split(';')[0].trim(),format=formats[mime];
  if(!format)throw new Error('Envie PNG, JPG, WebP, MP4, MP3, WAV, OGG ou PDF.');
