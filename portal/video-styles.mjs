...
export function normalizeVideoOptions(raw={}){
  ...
  return {preset:preset.id,aspectRatio:choose(raw.aspectRatio,['9:16','16:9'],'9:16'),quality:choose(raw.quality,['standard','high'],'high'),
    font:choose(raw.font,['sans','serif','condensed'],preset.font),fontSize,accent,
...
export function createVideoStyles({db}){
  ...
  async function save(org,user,{name,options,sourceCreationId,scenes=[]}){
    ...
    const duration=scenes.reduce((n,s)=>n+s.duration,0),recipe=scenes.slice(0,8).map(s=>({share:s.duration/duration,position:s.position||'center'}));
    const now=Date.now(),id=key,data={name,options:{...normalizeVideoOptions(options),musicAssetId:null},sourceCreationId,recipe,userId:user,version:1};
