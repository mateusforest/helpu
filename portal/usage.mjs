...
export async function usageSnapshot(db,org,policy,now=Date.now()) {
 ...
 return {day,timeZone:policy.timeZone||'America/Sao_Paulo',resetAt:usageResetAt(policy)||null,used,whatsappRepliesLast24Hours:Number(outbox.n),technicalUsage:await providerUsageSummary(db,org,now),limits:Object.fromEntries(['dailyRuns','dailyMedia','dailyMessages'].map(k=>[k,policy[k]]))};
}
