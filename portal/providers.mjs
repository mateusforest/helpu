...
export async function requestOpenAIResponse(config,body,{fetcher=fetch,sleep=ms=>new Promise(r=>setTimeout(r,ms))}={}) {
 ...
 for(let attempt=0;attempt<3;attempt++){
  ...
  if(response.ok)return {...data,requestId:response.headers.get('x-request-id')||null};
...
export function createProviders(fetcher=fetch){
 ...
 return {
  async generateImage(config,{prompt,size='1024x1024',images=[],onUsage}){
  ...
  async operationalValidation(config,{tools=[],onUsage}={}){
   ...
   try{
    ...
    data=await measuredResponse({method:'POST',headers:bearer(config.apiKey),body:{model,store:false,include:['reasoning.encrypted_content'],max_output_tokens:1600,input:prompt,tools:[probe],tool_choice:{type:'function',name:toolName},parallel_tool_calls:false}},onUsage);
