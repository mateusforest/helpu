...
export async function createConversation({db, dataDir, company, integration, list, saveRecord, queue, audit, json, assetPath, browserLaunch, respond, kernel, updateProfile, reviewPublication,integrationState,workerState,cloud=false,runtimeTools,browserOverride,deliveryOnly=true}) {
    ...
    try {
      while (iterations++ < 10) {
        ...
        const response = await ask(config, {
          ...
          instructions,
          input,
          tools: TOOLS.filter(t=>(!t.name.startsWith('browser_')||(!deliveryOnly&&(!cloud||!!browserOverride)))&&(!t.name.startsWith('video_')||runtimeTools?.configured)).map(t=>deliveryOnly&&t.name==='queue_action'?{...t,parameters:{...t.parameters,properties:{...t.parameters.properties,kind:{type:'string',enum:['agent','image','video']}}}}:t),
          parallel_tool_calls: false
        });
