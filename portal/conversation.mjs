...
const videoSettings={type:'object',properties:{preset:{type:'string',enum:['clean','editorial','dynamic','property','product']},aspectRatio:{type:'string',enum:['9:16','16:9']},quality:{type:'string',enum:['standard','high']},motion:{type:'string',enum:['none','zoom-in','zoom-out','pan']},textAnimation:{type:'string',enum:['fade','rise','pop','words']},font:{type:'string',enum:['sans','serif','condensed']},fontSize:{type:'number'},accent:string,transition:{type:'string',enum:['cut','fade','smoothleft']},fit:{type:'string',enum:['cover','contain']},soundEffects:{type:'string',enum:['none','subtle']},musicAssetId:string,musicVolume:{type:'number'},sourceAudio:{type:'boolean'}},additionalProperties:false};
