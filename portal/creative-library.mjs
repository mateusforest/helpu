...
export function createCreativeLibrary({db,metadata,saveMetadata,company}){
 ...
 async function context(org){const library=await list(org);return {...library,defined:hasVisualDirection(await company(org),await metadata(org,'brand:production'),library)};}
 async function promptContext(org){const c=await context(org);return {defined:c.defined,direction:c.direction,references:c.references.slice(0,4),styles:c.styles.slice(0,3)};}
 async function beforeCreate(org,thread,user,input,{text='',attachmentIds=[]}={}){
  ...
  return {needsReference:true,question:REFERENCE_QUESTION};
 }
