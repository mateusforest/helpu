...
export function createCreations({db,company,integration,queue,saveRecord,record,assetPath,storeAsset,systemUpdate,setJob,beforeMutation,renderer,respond,metadata,saveMetadata,fetcher=fetch}){
 ...
 async function capabilities(org){const config=await integration(org,'openai');return {videoPresets:VIDEO_PRESETS,techniques:VIDEO_TECHNIQUES,formats:['9:16','16:9'],qualities:['standard','high'],images:!!config.apiKey,reels:!!config.apiKey&&renderer.configured(),reelsReason:renderer.configured()?'':'O processamento de vídeo ainda não está disponível nesta versão da hospedagem.'};}
 ...
 async function submit(org,user,input,{conversationId,parentJobId,videoOptionKeys}={}){
  ...
  if(format==='reels'){const saved=input.styleId?await styles.get(org,input.styleId):null;request.videoOptions=normalizeVideoOptions({...saved?.options,...input.videoOptions});request.styleId=input.styleId||null;request.styleRecipe=saved?.recipe||null;request.explicitVideoKeys=videoOptionKeys??(saved?Object.keys(request.videoOptions):Object.keys(input.videoOptions||{}));request.referenceOnlyIds=input.referenceOnlyIds||[];if(!Array.isArray(request.referenceOnlyIds)||request.referenceOnlyIds.some(id=>!request.attachments.includes(id)))fail('Referência de estilo inválida.');}
  ...
  if(format==='reels'){const music=request.videoOptions.musicAssetId;if(music&&!checkedRefs.some(a=>a.id===music&&a.mime.startsWith('audio/')))fail('A música deve ser um áudio anexado da empresa.');const audio=checkedRefs.filter(a=>a.mime.startsWith('audio/'));if(audio.length>1)fail('Envie uma trilha de áudio por pedido.');if(!music&&audio.length)request.videoOptions.musicAssetId=audio[0].id;}
 ...
 async function plan(job,request){
  ...
  const isVideo=request.format==='reels',count=isVideo?(request.styleRecipe?.length||(request.duration===30?6:3)):request.slideCount;
  ...
  if(isVideo&&renderer.sampleReferences&&refs.some(a=>!a.mime.startsWith('audio/'))){
    ...
    const samples=await renderer.sampleReferences({assets});
